...
<div class="flex items-center justify-center gap-2 mt-6 fade-in" style="animation-delay: 250ms;">
    @if($currentPage > 1)
        <a href="{{ route('spv.utilization.index', ['page' => $currentPage - 1, 'status' => $statusFilter]) }}" 
    ...
    @for($i = 1; $i <= $totalPages; $i++)
        <a href="{{ route('spv.utilization.index', ['page' => $i, 'status' => $statusFilter]) }}" 
...
function openModal(id, name, status, keterangan) {
    ...
    document.getElementById('statusForm').action = `/spv/utilization/${id}`;
