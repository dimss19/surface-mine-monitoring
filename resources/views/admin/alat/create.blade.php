        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <select id="jenis" name="jenis" required class="block w-full rounded-xl border-outline-variant bg-surface-container text-on-surface shadow-sm focus:border-primary focus:ring-primary sm:text-sm px-4 py-2.5 transition-colors">
                    ...
                    <option value="excavator" {{ old('jenis') == 'excavator' ? 'selected' : '' }}>Excavator</option>
                    <option value="dump_truck" {{ old('jenis') == 'dump_truck' ? 'selected' : '' }}>Dump Truck</option>
                    <option value="dozer" {{ old('jenis') == 'dozer' ? 'selected' : '' }}>Dozer</option>
                    <option value="grader" {{ old('jenis') == 'grader' ? 'selected' : '' }}>Grader</option>
                    <option value="loader" {{ old('jenis') == 'loader' ? 'selected' : '' }}>Loader</option>
